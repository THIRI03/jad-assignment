...
@WebServlet("/CreateNewServiceForAdminServlet")
@MultipartConfig(
    fileSizeThreshold = 1024 * 1024 * 2, // 2MB threshold
    maxFileSize = 1024 * 1024 * 10,      // 10MB max file size
    maxRequestSize = 1024 * 1024 * 50    // 50MB max request size
)
public class CreateNewServiceForAdminServlet extends HttpServlet {
    ...
    public CreateNewServiceForAdminServlet() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean isCreated = serviceDAO.createService(service);

        if (isCreated) {
            request.setAttribute("message", "Service created successfully");
        } else {
